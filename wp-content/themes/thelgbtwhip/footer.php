          ...
          jQuery.ajax({
             ...
             success:function(data){
                ...
                for (var i = 0; i < arConstituency["candidates"].length; i++)
                {
                  ...
                  {
                    if (arConstituency["candidates"][i]["issues"][j]["issue_description"] == "Reduce age of consent for homosexual acts to 16")
                    {
                    
                    }
                    else if (arConstituency["candidates"][i]["issues"][j]["issue_description"] == "Introduction of civil partnerships")
                    {
                    
                    }
                    else if (arConstituency["candidates"][i]["issues"][j]["issue_description"] == "Equality Act (Sexual Orientation) Regulations")
                    {
                    
                    }
                    else if (arConstituency["candidates"][i]["issues"][j]["issue_description"] == "IVF treatment for lesbian couples")
                    {
                    
                    }
                    else if (arConstituency["candidates"][i]["issues"][j]["issue_description"] == "Marriage (Same-sex Couples) Bill")
                    {
                    
                    }
                    else if (arConstituency["candidates"][i]["issues"][j]["issue_description"] == "Reduce age of consent for homosexual acts to 16")
                    {
                    
                    }
                    else if (arConstituency["candidates"][i]["issues"][j]["issue_description"] == "Repeal of the 12 month deferral of men who have sex with men  (MSM) from donating blood.")
                    {
                    
                    }
                    else if (arConstituency["candidates"][i]["issues"][j]["issue_description"] == "Asylum in the UK for LGBT+ people fleeing persecution")
                    {
                    
                    }
                    else if (arConstituency["candidates"][i]["issues"][j]["issue_description"] == "Recognition of trans* identities in all public services")
                    {
